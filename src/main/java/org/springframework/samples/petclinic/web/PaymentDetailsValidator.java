...
public class PaymentDetailsValidator implements Validator {
	...
	public void validate(final Object target, final Errors errors) {
		...
		if (owner.getCreditCardNumber() != "" && owner.getExpirationMonth() != null && owner.getExpirationYear() != null
				&& owner.getCvv() != null) {
			...
			if (expMonth < 1 || expMonth > 12) {
				errors.rejectValue("expirationMonth", "The Month must be between 1 and 12",
						"The Month must be between 1 and 12");
			}

			if (expYear < 0) {
				errors.rejectValue(this.EXPIRATION_YEAR, "The Year must be positive", "The Year must be positive");
			}
			...
			if (!(expMonth < 1 || expMonth > 12) && expYear > 0) {
				...
				if (today.isAfter(expDate)) {
					errors.rejectValue(this.EXPIRATION_YEAR, "The Credit Card you introduced has expired",
							"The Credit Card you introduced has expired");
				}
			}

		} else {
			errors.rejectValue(this.EXPIRATION_YEAR, "You must fill all the fields", "You must fill all the fields");
		}
