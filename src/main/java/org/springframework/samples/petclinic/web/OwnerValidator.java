...
public class OwnerValidator implements Validator {
	...
	if (owner.getCreditCardNumber() != "" && owner.getExpirationMonth() != null && owner.getExpirationYear() != null
		...
		if (Integer.valueOf(cvv) < 0 || Integer.valueOf(cvv) > 999) {
		    errors.rejectValue("cvv", "CVV must be between 0 and 999", "CVV must be between 0 and 999");
		}
	    }
		...
		if (today.isAfter(expDate)) {
		    errors.rejectValue("expirationYear", "The Credit Card you introduced has expired",
