...
class HU05MakeOrder extends Simulation {
	...
	object MakeOrder {
		val makeOrder = exec(http("SHOWSC")
			.get("/shopping-cart")
			.headers(headers_0)
			.check(css("input[name=_csrf]", "value").saveAs("stoken")))
		.pause(20)
		.exec(http("MAKE ORDER")
	...
	setUp(scn_owner_1.inject(rampUsers(3500) during (100 seconds)),scn_owner_2.inject(rampUsers(3500) during (100 seconds))).protocols(httpProtocol).assertions(
