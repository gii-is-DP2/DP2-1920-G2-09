...
public class ProductValidatorTests {
	...
	void shouldNotValidateWhenNameEmpty() {
		...
		Errors errors = new BeanPropertyBindingResult(this.product, "");
		pv.validate(this.product, errors);
		Assertions.assertThat(errors.getFieldError("name").getCode()).isEqualTo("The name of the object cannnot be empty");
	}
	...
	void shouldNotValidateDescriptionIsLessThan20Characters() {
		...
		Errors errors = new BeanPropertyBindingResult(this.product, "");
		pv.validate(this.product, errors);
		Assertions.assertThat(errors.getFieldError("description").getCode()).isEqualTo("The lenght of the description must be between 20 and 128 characters");
	}

	@Test
	void shouldNotValidateDescriptionIsLongerThan128Characters() {
		this.product.setDescription("ESTA DESCRIPCIÓN ES DEMASIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAADO LARGA");
		ProductValidator pv = new ProductValidator();
		Errors errors = new BeanPropertyBindingResult(this.product, "");
		pv.validate(this.product, errors);
		Assertions.assertThat(errors.getFieldError("description").getCode()).isEqualTo("The lenght of the description must be between 20 and 128 characters");
	...
	void shouldNotValidateStockLessThan0() {
		...
		Errors errors = new BeanPropertyBindingResult(this.product, "");
		pv.validate(this.product, errors);
		Assertions.assertThat(errors.getFieldError("stock").getCode()).isEqualTo("The minimum stock possible is 0 units");
	}
	...
	void shouldNotValidateNegativeUnitPrice() {
		...
		Errors errors = new BeanPropertyBindingResult(this.product, "");
		pv.validate(this.product, errors);
		Assertions.assertThat(errors.getFieldError("unitPrice").getCode()).isEqualTo("The price of the product must be more than 0 euros");
	}
	...
	void shouldNotValidateZeroStockAndAvailable() {
		...
		Errors errors = new BeanPropertyBindingResult(this.product, "");
		pv.validate(this.product, errors);
		Assertions.assertThat(errors.getFieldError("stock").getCode()).isEqualTo("The product must have stock to be available");
	}
}
