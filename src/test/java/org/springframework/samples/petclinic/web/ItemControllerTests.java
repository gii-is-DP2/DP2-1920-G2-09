...
public class ItemControllerTests {
	...
	void testAddItemToShoppingCartSucces() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.post("/products/add-item/{productId}", 1).with(SecurityMockMvcRequestPostProcessors.csrf()).param("quantity", "2")).andExpect(MockMvcResultMatchers.status().is3xxRedirection())
	...
	void testAddItemToShoppingCartFailure() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.post("/products/add-item/{productId}", 1).with(SecurityMockMvcRequestPostProcessors.csrf()).param("quantity", "200")).andExpect(MockMvcResultMatchers.model().attributeExists("errorMessage"))
	...
	void testDeleteItemSuccess() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get("/products/delete-item/{itemId}", ItemControllerTests.TEST_ITEM_ID)).andExpect(MockMvcResultMatchers.status().is3xxRedirection())
	...
	void testDeleteItemFailure() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get("/products/delete-item/{itemId}", 4)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("exception"));
	}
	...
	void testEditItemSuccess() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.post("/products/edit-item/{itemId}", ItemControllerTests.TEST_ITEM_ID).with(SecurityMockMvcRequestPostProcessors.csrf()).param("quantity", "7"))
			.andExpect(MockMvcResultMatchers.status().is3xxRedirection()).andExpect(MockMvcResultMatchers.view().name("redirect:/shopping-cart"));
	}
	...
	void testEditItemFailure() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.post("/products/edit-item/{itemId}", ItemControllerTests.TEST_ITEM_ID).with(SecurityMockMvcRequestPostProcessors.csrf()).param("quantity", "50"))
			.andExpect(MockMvcResultMatchers.model().attributeExists("errorMessage")).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("/owners/ownerShoppingCart"));
	}
